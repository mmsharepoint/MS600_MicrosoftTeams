...
export const graphRouter = (options: any): express.Router => {
    ...
    const downloadTmpFileAsPDF = async (fileID: string, fileName: string, accessToken: string): Promise<any> => {
        ...
                              .then(response => {
                                  log(response);
                                  const respFile = { data: response.data,
                                                        name: fileName,
    ...
    const uploadFileToTargetSite = async (file: File, accessToken: string, domain: string, siteRelative: string, 
        channelName: string): Promise<string> => {
...
                const pdfFile = await downloadTmpFileAsPDF(tmpFileID, filename, accessToken);
                const webUrl = await uploadFileToTargetSite(pdfFile, accessToken, req.body.domain,
                                  req.body.sitepath, req.body.channelname);
                res.end(webUrl);
